...
package through the `cropr_simulation` data format (see [section 13](#13-data-structure)).
...
(see [section 11](#11-data-manipulation)).
...
Overlay observed data on simulated outputs for model validation:
...
Visualize model residuals (observed - simulated) using `select_scat = "res"`:
...
Suffix the variable name with `_obs` or `_sim` to choose observed or simulated values:
