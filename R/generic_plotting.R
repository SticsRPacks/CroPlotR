...
plot_generic_situation <- function(sim,obs=NULL,obs_sd=NULL,
  ...
  if(is_obs && !is.null(obs$Plant) && !is.null(sim$Plant)){
    ...
    if(any(!common_crops)){
      cli::cli_alert_warning(paste0("Observed and simulated crops are different.
                                    Obs Plant: ",
  ...
  formated_df <- formater(sim, obs, obs_sd, type, select_dyn, select_scat,
                        all_situations, successive=successive,
  ...
  if(type=="dynamic" && !is.null(overlap) && (total_vers>1) &&
    ...
    formated_df <-
      dplyr::bind_cols(formated_df,
                       data.frame("Combi"=
                    paste(rep(paste0("Version_",num_vers),nrow(formated_df)),
  ...
  if(type=="dynamic"){
    situation_plot <-
      formated_df%>%
      ggplot2::ggplot(ggplot2::aes(y= .data$Simulated, x= .data$Date,
                                   linetype= !!aesth$linetype[[1]],
                                   shape= !!aesth$shape[[1]],
                                   color= !!aesth$color[[1]]))+
                                    ggplot2::geom_line(na.rm = TRUE)+
      ggplot2::labs(color= names(aesth$color),linetype= names(aesth$linetype),
    ...
    if(select_scat=="sim"){
      situation_plot <-
        ...
        ggplot2::ggplot(ggplot2::aes(y= .data$Simulated, x= .data$Observed,
                                     shape= !!aesth$shape[[1]],
                                     linetype = !!aesth$linetype[[1]],
                                     color= !!aesth$color[[1]],
                                     text = .data$Sit_Name)) +
    ...
    if(select_scat=="res") {
      situation_plot <-
        if(is.null(reference_var)){
          formated_df%>%
          ggplot2::ggplot(ggplot2::aes(y= .data$Observed - .data$Simulated,
                                       x= .data$Observed,
                                       shape= !!aesth$shape[[1]],
                                       linetype = !!aesth$linetype[[1]],
                                       color= !!aesth$color[[1]],
      ...
      situation_plot <- situation_plot +
        ...
        ggplot2::geom_point(na.rm=TRUE)+
        ggplot2::geom_abline(intercept = 0, slope = 0, color= "grey30",
                             linetype= 2)+
    ...
    situation_plot <- situation_plot+
      formated_df%>%
      ggplot2::labs(shape= names(aesth$shape),linetype=names(aesth$linetype),
...
plot_situations <- function(...,obs=NULL,obs_sd=NULL,
  ...
  if(length(dot_args)>1){
    for(indice in 2:length(dot_args)){
      common_situations_models <- intersect(common_situations_models,
                                            names(dot_args[[indice]]))
    }
  }
  ...
  general_plot <-
    lapply(common_situations_models, function(x){
      sim_plot <-
        plot_generic_situation(sim = dot_args[[1]][[x]], obs = obs[[x]],
      ...
      if(!is.null(sim_plot)){
        ...
        aesth <- aesthetics(dot_args[[1]][[x]],obs[[x]],type=type,
                            overlap=overlap,several_sit=several_sit,
                          shape_sit=shape_sit,
                          one_version=(length(dot_args)==1))$versions
        ...
        if(type=="dynamic"){
          sim_plot$layers[[1]] <-
            if(is.null(aesth$linetype[[1]]) && length(V_names) == 1){
              ...
              ggplot2::geom_line(ggplot2::aes_(color= aesth$color[[1]],
                                               linetype= aesth$linetype[[1]]),
                                 na.rm = TRUE)
            }
          ...
          if(!is.null(aesth$shape[[1]]) && !is.null(obs[[x]]) &&
             (nrow(obs[[x]])>0)){
            sim_plot$layers[[2]] <-
              ggplot2::geom_point(ggplot2::aes_(y= sim_plot$data$Observed,
                                                color= aesth$color[[1]],
                                                shape= aesth$shape[[1]]),
                                  na.rm = TRUE)
          }
          ...
          if(!is.null(obs_sd[[x]]) && (nrow(obs_sd[[x]])>0)){
            sim_plot$layers[[3]] <-
              ...
              ggplot2::aes_(ymin=sim_plot$data$Observed-2*sim_plot$data$Obs_SD,
          ...
          sim_plot$layers[[3]] <-
            if(is.null(aesth$linetype[[1]])){
              ...
              ggplot2::geom_smooth(ggplot2::aes_(linetype= aesth$linetype[[1]],
                                                 group=1),
                                   method=lm,colour="blue",se=FALSE,size=0.6,
                                   formula= y ~ x,fullrange=TRUE,na.rm=TRUE)
            }
  ...
  for(iVersion in seq_along(dot_args)){
    ...
    for(j in common_situations_models){
      tmp <-
        plot_generic_situation(sim = dot_args[[iVersion]][[j]], obs = obs[[j]],
      ...
      aesth <- aesthetics(dot_args[[iVersion]][[j]],obs[[j]],type=type,
                          overlap=overlap,several_sit=several_sit,
                        shape_sit=shape_sit,one_version=FALSE,iVersion=iVersion,
                        dot_args=dot_args)$versions
      ...
      general_plot[[j]] <-
        general_plot[[j]] +
        if(type=="dynamic"){
          ggplot2::geom_line(data = tmp, ggplot2::aes_(color= aesth$color[[1]],
                                                linetype= aesth$linetype[[1]]),
                             na.rm = TRUE)
        }else{
      ...
      if(!is.null(obs[[j]]) && (nrow(obs[[j]])>0) && type=="dynamic"){
        general_plot[[j]] <- general_plot[[j]] +
          ggplot2::geom_point(data = tmp, ggplot2::aes_(y= tmp$Observed,
                                                        color= aesth$color[[1]],
                                                      shape= aesth$shape[[1]]),
                              na.rm = TRUE)
        if(!is.null(obs_sd[[j]]) && (nrow(obs_sd[[j]])>0)){
          general_plot[[j]] <- general_plot[[j]] +
            ggplot2::geom_errorbar(data = tmp,
                                   ggplot2::aes_(ymin=tmp$Observed-2*tmp$Obs_SD,
      ...
      if(shape_sit=="txt" && type=="scatter"){
        general_plot[[j]] <- general_plot[[j]]+
          ggrepel::geom_text_repel(data = tmp,
                                   ggplot2::aes_(label=tmp$Sit_Name,
                                                 color= aesth$color[[1]]),
                                 na.rm = TRUE, show.legend = FALSE)
      }
...
plot.statistics <- function(x,xvar=c("group","situation"),type=c("bar","radar"),
  ...
  x <-
    x%>%
    reshape2::melt(id.vars= c("group","situation","variable"),
  ...
  if(type=="bar"){
    ...
    x <-
      ...
      ggplot2::ggproto("CoordRadar",
                       ggplot2::CoordPolar, theta="x", r="y", start=-pi/6,
