...
sim= SticsRFiles::get_daily_results(workspace = workspace, usm_name = usms)
obs= SticsRFiles::get_obs(workspace =  workspace, usm_name = usms)
plot_usms(sim,obs)

test_that("format of ploting several situations", {
  test_plot= plot_usms(sim,obs)
